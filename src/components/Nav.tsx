...
const Nav = () => {
  ...
  const userEmail = user?.email.split('@') || 'Invitado';
  ...
  return (
    <nav className='flex flex-col md:flex-row gap-4 justify-evenly items-center'>
      <h2 className='text-2xl py-6'>Hola {Array(userEmail) ? userEmail[0] : userEmail}!</h2>

      <ul className='flex text-sm gap-4'>
        <Link to={`/scores/${user?.id}`}><li className='flex hover:bg-gray-700 cursor-pointer px-4 py-2 bg-gray-600 rounded-full gap-2 items-center'>Mis puntajes <svg xmlns="http://www.w3.org/2000/svg" width={24} height={24} viewBox="0 0 50 50">
